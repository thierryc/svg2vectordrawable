...
let JS2XML = function() {
    ...
    this.vectordrawableAttrs = [
        ...
        'android:pivotX',
        'android:pivotY',
        'android:scaleX', // scale(<x> [<y>])
        'android:scaleY',
        'android:translateX', // translate(<x> [<y>])
        'android:translateY',
        // <path>
        'android:pathData', // d
...
JS2XML.prototype.refactorData = function(data) {
    ...
    elemGroups.forEach(elem => {
        ...
        if (elem.hasAttr('transform')) {
            ...
            // console.log(translateRegExp.exec(svgTransform))
            // console.log(scaleRegExp.exec(svgTransform))
            console.log(rotateRegExp.exec(svgTransform))
        }
    ...
    if (elemMasks) {
        elemMasks.forEach(elem => {
            if (elem.hasAttr('id')) {
                ...
                elem.parentNode.spliceContent(elem.parentNode.content.indexOf(maskedElems[maskedElems]), maskedElems.length, maskGroup);
            }
        });
    }
...
JS2XML.prototype.createAttrs = function(elem) {
    ...
    elem.eachAttr(function(attr) {
        if (attr.value !== undefined && this.vectordrawableAttrs.indexOf(attr.name) !== -1) {
