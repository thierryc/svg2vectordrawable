...
const pathBounds = require('svg-path-bounds'); // https://www.npmjs.com/package/svg-path-bounds
...
let JS2XML = function() {
    ...
    this.vectordrawableTag = [
        'vector', 'group', 'path', 'clip-path', 'gradient', 'item', 'aapt:attr'
    ]
};
...
JS2XML.prototype.refactorData = function(data) {
    ...
    elemPaths.forEach(elem => {
        ...
        // Android not support path/group alpha
...
JS2XML.prototype.convert = function(data) {
    ...
    xml += EOL + this.travelConvert(data);
    return xml;
};
...
JS2XML.prototype.createElement = function(data) {
    ...
    // console.log(Object.keys(this.tagConvert).indexOf(data.elem))
    ...
    if (data.isEmpty()) {
        ...
        // if (Object.keys(this.tagConvert).indexOf(data.elem) >= 0) {
